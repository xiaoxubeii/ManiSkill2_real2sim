...
from mani_skill2_real2sim.agents.robots.rontqirobot import RongqiRobot
...
class CustomSceneEnv(BaseEnv):
    SUPPORTED_ROBOTS = {"google_robot_static": GoogleRobotStaticBase,
                        ...
                        "rongqirobot": RongqiRobot,
                        }
    agent: Union[GoogleRobotStaticBase, WidowX, Panda, RongqiRobot]
    ...
    def __init__(
        ...
        **kwargs
        ...
        if rgb_overlay_path is not None:
            ...
            self.rgb_overlay_img = cv2.cvtColor(cv2.imread(
                rgb_overlay_path), cv2.COLOR_BGR2RGB) / 255  # (H, W, 3); float32
        else:
        ...
        # perform "greenscreen" on the specified camera(s) observations
        self.rgb_overlay_cameras = rgb_overlay_cameras
        # 'background' or 'object' or 'debug' or combinations of them
        self.rgb_overlay_mode = rgb_overlay_mode
        # always overlay / greenscreen these objects regardless of the rgb_overlay_mode
        self.rgb_always_overlay_objects = rgb_always_overlay_objects
        assert ('background' in self.rgb_overlay_mode) or (
            'debug' in self.rgb_overlay_mode), 'Invalid rgb_overlay_mode'
    ...
    def _load_arena_helper(self, add_collision=True):
        ...
        if self.scene_name is None:
            ...
            # no scene; we will add a dummy scene with ground and optionally a fake tabletop
            scene_path = None
        else:
            scene_path = str(self.scene_root / "stages" /
                             f"{self.scene_name}.glb")
        ...
        if self.scene_name is not None:
            ...
            if "modern_bedroom" in self.scene_name:
                ...
                scene_pose = sapien.Pose(
                    [0.178, -2.235, 1.669], [0.007, 0, 0, -1]) * scene_pose
            elif "modern_office" in self.scene_name:
                scene_offset = np.array([-1.6616, -3.0337, 0.0])
                scene_pose = sapien.Pose(
                    [-0.192, -1.728, 1.48], [0.709, 0, 0, -0.705]) * scene_pose
    ...
    def _initialize_agent(self):
        ...
        if "google_robot_static" in self.robot_uid:
            ...
            if self.robot_uid in ['widowx', 'widowx_bridge_dataset_camera_setup']:
                qpos = np.array([-0.01840777,  0.0398835,   0.22242722,  -
                                0.00460194,  1.36524296,  0.00153398, 0.037, 0.037])
            ...
            robot_init_height = 0.0  # Adjust this value as needed
            robot_init_rot_quat = [0, 0, 0, 1]
        else:
            raise NotImplementedError(self.robot_uid)
        ...
        if (robot_init_xy := self.robot_init_options.get("init_xy", None)) is not None:
            robot_init_xyz = [robot_init_xy[0],
                              robot_init_xy[1], robot_init_height]
        else:
        ...
        self.agent.robot.set_pose(sapien.Pose(
            robot_init_xyz, robot_init_rot_quat))
    ...
    def get_obs(self):
        ...
        if self._obs_mode == "image" and self.rgb_overlay_img is not None:
            ...
            target_object_actor_ids = [x.id for x in self.get_actors() if x.name not in [
                'ground', 'goal_site', '', 'arena'] + self.rgb_always_overlay_objects]
            target_object_actor_ids = np.array(
                target_object_actor_ids, dtype=np.int32)
            ...
            robot_link_ids = np.array(
                [x.id for x in robot_links], dtype=np.int32)
            ...
            for camera_name in self.rgb_overlay_cameras:
                ...
                assert 'Segmentation' in obs['image'][camera_name].keys(
                ...
                if ('background' in self.rgb_overlay_mode) or ('debug' in self.rgb_overlay_mode):
                    if ('object' not in self.rgb_overlay_mode) or ('debug' in self.rgb_overlay_mode):
                        ...
                        mask[np.isin(actor_seg, np.concatenate(
                            [robot_link_ids, target_object_actor_ids, other_link_ids]))] = 0.0
                    else:
                ...
                rgb_overlay_img = cv2.resize(self.rgb_overlay_img, (
                    obs['image'][camera_name]['Color'].shape[1], obs['image'][camera_name]['Color'].shape[0]))
                if 'debug' not in self.rgb_overlay_mode:
                    obs['image'][camera_name]['Color'][..., :3] = obs['image'][camera_name]['Color'][...,
                                                                                                     :3] * (1 - mask) + rgb_overlay_img * mask
                else:
                    # debug
                    # obs['image'][camera_name]['Color'][..., :3] = obs['image'][camera_name]['Color'][..., :3] * (1 - mask) + rgb_overlay_img * mask
                    obs['image'][camera_name]['Color'][...,
                                                       :3] = obs['image'][camera_name]['Color'][..., :3] * 0.5 + rgb_overlay_img * 0.5
    ...
    def _get_instruction_obj_name(s):
        ...
        s = s.split('_')
        rm_list = ['opened', 'light', 'generated',
                   'modified', 'objaverse', 'bridge', 'baked', 'v2']
        cleaned = []
        for w in s:
            if w[-2:] == "cm":
